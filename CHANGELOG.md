...
Type Inspection annotations:
* Add `InspectPatternMatches` and `InspectAllocations` annotations for
  inspecting types pattern-matched (scrutinized) and allocated (constructed)
  within a binding.
* Add `InspectTypeClasses` annotations for type class dictionary inspection
  within a binding.

Core Inspection annotations:
